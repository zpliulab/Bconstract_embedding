...
class node_embedding:
    ...
    def get_node_embeds(self):
        ...
        rna_embeds = []
        for item in rna_id:
            rna_embeds.append(map[str(item)])
        pro_embeds = []
        for item in pro_id:
            pro_embeds.append(map[str(item)])
...
class edge_embedding:
    ...
    def get_edge_embeds(self):
        ...
        neg_test_edges = []
        for item in neg_test_index:
            pro_index = item%59
            rna_index = item // 59
            neg_test_edges.append(rna_id[rna_index]+' '+pro_id[pro_index]+'\n')
        return np.array(pos_train_feas.data), np.array(pos_test_feas.data), np.array(neg_train_feas.data), np.array(neg_test_feas.data), np.array(neg_test_edges)
