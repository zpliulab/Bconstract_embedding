...
spe11= 0
for i in range(3):
    path = os.path.join('data/4/dataset', str(i+1))
    data = split_train_test('data/4/inters.txt')
    test_pos_edges, train_num, test_num, labels = data.load(i)
    node_feas = node_embedding('data/4/inters.txt', f'{path}/embeds')
    rna_embeds, pro_embeds = node_feas.get_node_embeds()
    rna_embeds = rna_embeds.astype(float)
    pro_embeds = pro_embeds.astype(float)
    edge_feas = edge_embedding(rna_embeds, pro_embeds, train_num.item(), test_num.item(), test_pos_edges, labels, 'data/4/inters.txt')
...
np.save('data/4/dataset/y_true.npy', y_true)
np.save('data/4/dataset/y_predict.npy', y_predict)
