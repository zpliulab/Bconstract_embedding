...
class node_embedding:
    ...
    def get_node_embeds(self):
        ...
        _, _, _, rna_int, pro_int = inters_id.get_iters_rna_id_pro_id()
        rna_embeds = []
        for item in rna_int:
...
class edge_embedding:
    ...
    def get_pos_neg_index(self):
        ...
        for item in self.testing_pos_edges:
            l_id, p_id = item
            p_index = p_id-len(rna_int)
            pos_test_index.append(int(l_id*len(pro_int)+p_index))
            pos_index.remove(l_id*len(pro_int)+p_index)
